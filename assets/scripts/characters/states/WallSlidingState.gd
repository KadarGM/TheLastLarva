...
func process_input() -> void:
	...
	if character.big_jump_charged and input.dash:
		if input_direction != 0 and sign(input_direction) == sign(wall_direction):
			character.execute_big_jump(Vector2(wall_direction, 0))
			var big_jump_state = state_machine.states.get("BigJumpingState")
			if big_jump_state:
				big_jump_state.set_direction(Vector2(wall_direction, 0))
			state_machine.transition_to("BigJumpingState")
			return
	
	if input.jump_pressed and character.can_wall_jump and not character.big_jump_charged:
		execute_wall_jump()
		return
	...
	if input.dash and character.can_big_jump and character.timers_handler.big_jump_cooldown_timer.is_stopped() and not input.charge_jump:
