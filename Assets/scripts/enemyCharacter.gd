...
func initialize_character() -> void:
	if capabilities.has_detection_area and shape_detection:
...
func connect_additional_signals() -> void:
	if detection_area and capabilities.has_detection_area:
...
func process_character(_delta: float) -> void:
	...
	if player and capabilities.has_view_ray and view_ray:
...
func handle_state_transitions() -> void:
	...
	if current_state == State.JUMPING:
		if is_on_floor() and has_jumped:
			has_jumped = false
			if player_in_detection_zone and can_see_player and capabilities.can_chase:
				change_state(State.CHASING)
			else:
				change_state(State.WALKING)
		return
	
	if player_in_attack_range and target_player and is_instance_valid(target_player) and capabilities.can_attack:
		if not attack_cooldown_timer or attack_cooldown_timer.is_stopped():
			change_state(State.ATTACKING)
		return
	
	if player_in_detection_zone and target_player and is_instance_valid(target_player) and can_see_player and capabilities.can_chase:
		var y_difference = global_position.y - target_player.global_position.y
		
		if y_difference > 0 and y_difference < abs(character_data.jump_velocity) * 0.8 and is_on_floor() and chase_direction != 0 and capabilities.can_jump:
		...
		if current_state == State.CHASING or current_state == State.IDLE:
			if capabilities.can_patrol and between_states_timer and not between_states_timer.time_left > 0:
...
func start_patrol_behavior() -> void:
	if not player_in_detection_zone and current_state != State.DEATH and current_state != State.ATTACKING and current_state != State.JUMPING and capabilities.can_patrol:
...
func enter_state(state: State) -> void:
	...
	match state:
		State.JUMPING:
			if previous_state == State.WALKING or previous_state == State.CHASING:
...
func _on_detection_area_body_exited(exited_body: Node2D) -> void:
	if exited_body == target_player:
		...
		if current_state != State.DEATH and current_state != State.ATTACKING and capabilities.can_patrol:
			start_patrol_behavior()
...
func _on_between_states_timer_timeout() -> void:
	if not player_in_detection_zone and current_state != State.DEATH and current_state != State.ATTACKING and current_state != State.JUMPING and capabilities.can_patrol:
		if current_state == State.IDLE:
			...
			if randf() < capabilities.patrol_chance_idle:
				change_state(State.IDLE)
			else:
				if randf() < 0.5:
					movement_direction *= -1
